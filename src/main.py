...
# shared state between UI and inference loop
target = ""
running = False
...
class LockLensApp:
    ...
    def build_ui(self):
        # main layout — left camera, right panel
        ...
        tk.Label(logo_frame, text="⬤", fg=self.pink, bg=self.navy, font=("Arial", 8)).pack(side=tk.LEFT)
        tk.Label(logo_frame, text=" LockLens", fg=self.white, bg=self.navy, font=("Arial", 13, "bold")).pack(side=tk.LEFT)

        # status pill
        self.status_var = tk.StringVar(value="ready")
        self.status_label = tk.Label(self.right, textvariable=self.status_var,
            fg=self.pink, bg=self.card, font=("Arial", 9, "bold"),
            padx=10, pady=4)
        ...
        self.make_metric(metrics, "LATENCY", self.latency_var, "ms", 0, 0)
        ...
        tk.Label(target_card, text="TRACKING TARGET", fg=self.muted,
            bg=self.card, font=("Arial", 8)).pack(anchor=tk.W, padx=8, pady=(6, 0))
        self.target_display = tk.Label(target_card, text='"--"', fg=self.white,
            bg=self.card, font=("Arial", 11))
        self.target_display.pack(anchor=tk.W, padx=8)
        self.target_entry = tk.Entry(target_card, bg="#0D1117", fg=self.white,
            insertbackground=self.white, font=("Arial", 10), relief=tk.FLAT,
            highlightthickness=1, highlightcolor=self.pink)
        ...
        self.target_entry.bind("<FocusIn>", lambda e: self.target_entry.delete(0, tk.END))

        # buttons
        btn_frame = tk.Frame(self.right, bg=self.navy)
        btn_frame.pack(fill=tk.X, padx=14, pady=(8, 0))
        tk.Button(btn_frame, text="START", bg=self.pink, fg=self.white,
            font=("Arial", 11, "bold"), relief=tk.FLAT, padx=0, pady=10,
            command=self.start_tracking).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 4))
        tk.Button(btn_frame, text="STOP", bg=self.card, fg=self.pink,
            font=("Arial", 11, "bold"), relief=tk.FLAT, padx=0, pady=10,
            command=self.stop_tracking).pack(side=tk.LEFT, fill=tk.X, expand=True)

        # amd badge
        tk.Label(self.right, text="AMD MI300X · ROCm 7.2 · vLLM",
            fg=self.muted, bg=self.navy, font=("Arial", 8)).pack(side=tk.BOTTOM, pady=8)

    def make_metric(self, parent, label, var, unit, row, col):
        frame = tk.Frame(parent, bg=self.card)
        frame.grid(row=row, column=col, padx=3, pady=3, sticky="nsew")
        parent.columnconfigure(col, weight=1)
        tk.Label(frame, text=label, fg=self.muted, bg=self.card,
            font=("Arial", 8)).pack(anchor=tk.W, padx=8, pady=(6, 0))
        tk.Label(frame, textvariable=var, fg=self.pink, bg=self.card,
            font=("Courier", 16, "bold")).pack(anchor=tk.W, padx=8, pady=(0, 6))
    ...
    def update_ui(self):
        ...
        colors = {"tracking": "#FF375F", "searching": "#BF9FFF", "stopped": "#9FE0FF", "ready": "#BDC8E0"}
...
if __name__ == "__main__":
    ...
    # launch tkinter UI on main thread
    root = tk.Tk()
